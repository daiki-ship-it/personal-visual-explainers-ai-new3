...
| **公開 URL（ブラウザ確認用）** | **このセルのみが正**（他ファイルでは URL を書かず、本表を指す）。Surge 等へ載せたら **ここに公開 URL を1本だけ**追記する。ローカル確認: リポジトリ直下の `output/ai-three-mar24-2026/index.html`（マスターと同型のとき）。アンカー例: `#section-map` / `#section-policy` |
| **output との同期例（任意）** | `output/ai-three-mar24-2026/index.html` — マスター更新後は必要に応じてコピーで揃える |
...
- **全体の地図**: `h2`（Lucide + 見出し）→ リード文 → `grid gap-4 md:grid-cols-2` のカード（番号付きタイトル + 短い説明。本スキル既定の「講師3本」では各カードを **`md:col-span-2` で縦に3つ**並べる。月別の広い俯瞰などでは `md:col-span-2` と 1 列カードの組み合わせも可）→ 下に**たとえ話の締め**を1段落。
- **用語**: **1 セクション**に `term-explain` を縦に積む（見本は「先に押さえる用語」+ `lightbulb` の `h2`）。中身の語彙ルールは term-dictionary のみが正。
- **まず覚える3つ**: `check-circle` の `h2` → リード → `space-y-4` の行（`badge-essential` / `badge-recommended` / `badge-optional` + 見出し + 補足）。ニュース型では **「何が起きたか」「講師との関係」** を各行に含める（見本の `#section-three`）。**3行の中身の意味**（単一題材の3要点か、最新ニュースの講師向け3本か）は [SKILL.md](../SKILL.md) の「『まず覚える3つ』との対応」に従う。
- **本編各セクション**（トピックごと）: `section-card` ごとに `h2`（Lucide + タイトル）→ **学習者対話（`mb-6`）→ ナビゲーター対話（`mb-6`）** → 本文（箇条書き・`rounded-lg border` のまとめ枠・2 カラムのミニ図・`<details>` など、見本にあるパターンから選ぶ）→ 必要なら `text-xs` の参考リンク行。**本スキル既定（最新3本）**では採用トピックが **ちょうど3つ**なので `section-policy` / `section-oss` / `section-models` のように **本編3セクション**（`id` の命名はトピックに合わせてよい）。
...
3. 「全体の地図」（見本: グリッドカード + 締めのたとえ1段落）
   └ 公式の見出しをそのまま並べず、自分向けに再分割。**本スキル既定**は **採用3本＝地図3ブロック**（各 `md:col-span-2`）。月別の広い俯瞰などでは3〜5ブロックでもよい。
...
6. 各トピック section-card（本スキル既定では **採用3本＝本編3セクション**）
...
- [ ] **構造・レイアウト**が [exemplar-layout.html](exemplar-layout.html) と同型：導入1カード、地図グリッド、用語1セクション、まず覚える3つ、本編は h2→対話2回→本文、まとめ、出典+免責、目次対応
...
- [ ] 「まず覚える3つ」または同等の強い絞り込みがあり、**見本と同じバッジ行レイアウト**になっている
...
- [ ] 最後に学習者が**自分の言葉**で要約している（成功の定義は [SKILL.md](../SKILL.md) の「読者・仕事・ゴール・『まず覚える3つ』の意味」→「成功の定義（ゴール）」）
